...
var customSucrette = {
    "avatar":{
        "skin":"103-f95bf0dc4ab32d9d", 
        "customSkin":null,
        "hairStyle":"43-25ef6acfc737bb98",
        "hairColor":"16-929208fc271f38c9", 
        "eyebrow":"73-5ed5dc220736237e", 
        "eyeType":"74-e2ba5f531efc60c5", 
        "eyeColor":"29-fe49a2ea234ae71b_no", 
        "mouth":"75-98d2f64e7d805a7e_no", 
        "makeUp":null
    },
    "clothe":[
        {"category":"underwears", "src":"https://assets3.corazondemelon.es/clothe/web/thumb_MD/74-9813e23a4194121e.png"}
    ],
    "maxCategory":[
        ...
        {"category":"highHeels", "max":1, "type":"replace"},
...
const requestAvatar = new XMLHttpRequest();requestAvatar.open("GET", "../data/v1/avatar.json");
requestAvatar.responseType = "json";requestAvatar.send();requestAvatar.onload = function() {

    const requestInfo = new XMLHttpRequest();requestInfo.open("GET", "../data/v1/info.json");
    requestInfo.responseType = "json";requestInfo.send();requestInfo.onload = function() {

        const requestClothe = new XMLHttpRequest();requestClothe.open("GET", "../data/v1/clothe.json");
...
function cargarAvatar(page = 0) {
    ...
    for (b = page * 8; b < base.length; b++) {
        ...
        if (categoria == "hairStyle") {
            ...
            url += base[b].id + "_" + customSucrette.avatar.eyeColor + ".png";
            if (customSucrette.avatar.eyeType == base[b].id) selected = "selected";

        } else if (categoria == "eyeColor") {
            url += customSucrette.avatar.eyeType + "_" + base[b].id + ".png";
...
async function drawSucrette(size = "body") {
    ...
    (heelsOn) ? img = img.replace("thumb", "heels") : img = img.replace("thumb", "normal");
    if (size == "body" || size == "small") { img = img.replace("_MD", "");
    } else if (size == "face" || size == "big") { img = img.replace("_MD", "_HD") };
    ...
    // -------------------------------------------------
    var baseIMG = "https://assets3.corazondemelon.es/avatarpart/web/";
    heelsOn ? baseIMG += "heels" : baseIMG += "normal";
    ...
    // Cargar ojos
    var img = baseIMG + customSucrette.avatar.eyeType + "_" + customSucrette.avatar.eyeColor + ".png";
    var ready = await preloadIMG(img);
    ...
    // Cargar boca
    var img = baseIMG + customSucrette.avatar.mouth + ".png";
    var ready = await preloadIMG(img);
...
$(function() {
    ...
    $(".showcase").on("click", ".declinable-avatar", function() {
        ...
        } else if (categoria == "eyeColor") {customSucrette.avatar.eyeColor = temp[1] + "_no";
        } else if (categoria == "eyebrow") {customSucrette.avatar.eyebrow = temp[0];
        } else if (categoria == "mouth") {customSucrette.avatar.mouth = temp[0] + "_no";
...
function categoriaSeleccionada() {
    ...
    if (name == "category-HighHeels") return "highHeels";
...
function categoriaNombre() {
    ...
    if (name == "category-HighHeels") return "Tacones";
